...
- **`init` synthetic do_backup deferred (§10.1 step 7).** The `init` subcommand does not run a synthetic `do_backup` at bootstrap time. Operator's first-deploy procedure is: (1) run `init`, (2) immediately run `mthydra-controller backup-now` to verify the pipeline end-to-end before enabling the systemd unit. This two-step procedure is equivalent to the spec's intent; the `backup-now` command was a print stub at the time step 7 was deferred, and is now a real synchronous pipeline call.
- **Cross-spec obligation IDs seeded but unrefreshable until those specs ship.** `init` seeds `obligation_clocks` rows for `t1_dormant_health`, `t2_dryrun_caseA`, `t2_dryrun_caseB`, `t3_vantage_revalidation`, `t3_profile_repin`, `t4_upstream_check`, `t5_pool_revalidation`, `t6_reshuffle`. These IDs are owned by specs B–H; only spec A owns `backup_restore_dryrun`. Operators will see 8 stale clocks until those specs ship and provide the runbooks to prove them. The `obligation-proven` CLI can manually stamp them; the `next_due_at` will be computed from `controller.toml [obligations.timers_hours]` which is pre-populated in `controller.toml.example`.
- **`index.json` retention uses GOVERNANCE, not COMPLIANCE (G7 resolution).** Backup blobs (`gen-*.age`) are PUT under `ObjectLockMode=COMPLIANCE` — immutable, no operator override. `index.json` versions are PUT under `ObjectLockMode=GOVERNANCE` — the operator can delete or replace a corrupted index version using an S3 bypass with elevated credentials. This asymmetry is intentional: the operator must never be able to rewrite backup history (blob COMPLIANCE) but must be able to recover from a corrupted index pointer (index GOVERNANCE). Retention period is `object_lock_days` in both cases.
- **No structured logging in spec A (deferred to spec J).** All output goes to stdout/stderr, captured by the systemd journal. The filesystem paths `/var/lib/mthydra/logs/controller.log` and `/var/lib/mthydra/logs/backup.log` named in §3 are reserved but not written by spec A code. Spec J (observability service) will introduce structured logging. The `audit.log` mirror file (§4.7) is the one exception — it is written by spec A code.
- **Bucket creation is an operator pre-step.** The controller has no `create-bucket` subcommand. B2 Object Lock + Versioning + lifecycle policy must be applied at bucket-creation time using the B2 console or API directly, before running `init`. This is noted in the README deployment section.
- **`systemd-analyze verify` not confirmed.** The service units in `packaging/systemd/` were not passed through `systemd-analyze verify` because doing so requires root access on an Ubuntu 24.04 host. Operator should run this check before deploying the units.
