...
9. **Compromise on an already-retired shard is a no-op.** If a TTL-reshuffle already happened between box-live and box-compromise, the box's shard_id (preserved by the H-D2 trigger) points at a retired shard. The compromise hook detects this and emits no new reshuffle: the dangerous user set has already moved to a different shard, so re-churning would only cost. Bounded residual: the *new* shard's exposure window started at the reshuffle, not the original compromise. Acceptable because the post-reshuffle shard composition is fresh.
10. **`shard-reshuffle` and the compromise hook read `target_size` from the shard row** (the `shards.target_size` column captured at shard creation) rather than from `cfg.shard_manager.target_size`. This is a deliberate deviation from §2 H-D4 wording: shards remember the policy in effect when they were created, so changing the global config does not retroactively destabilize existing shards. The scheduler still uses the live config for *new* shards from unassigned-fold-in.
11. **Bootstrap placeholder `t6_reshuffle` removed.** The pre-spec-H bootstrap seeded an obligation called `t6_reshuffle` with a 168h placeholder cadence. Spec H replaces this with three formal obligations (`shard_reshuffle_proven` 28d, `shard_reshuffle_sweep_ran` 1h, `shard_disjointness_check_proven` 24h). Operators upgrading existing controllers will find an orphan `t6_reshuffle` row; it is safe to leave (no consumer references it) or remove via `obligation-proven`. The deployment runbook should mention this.
