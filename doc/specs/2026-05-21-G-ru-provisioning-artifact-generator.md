...
|---|---|---|
| G-D1 | **Scope = seed generation + close spec A's authority placeholder.** Provider-API VM provisioning deferred to spec G2. | Tunnel-software-style sysadmin work belongs in its own spec, mirroring the F → F2 split. G1 ships a real, testable bundle the operator can paste into a provider console; G2 automates the paste. (**Note:** target providers are Russian / CIS-adjacent — Selectel, Timeweb, FirstVDS, VK Cloud, etc. Hetzner/AWS/GCP do not operate inside the cordoned RU network and are not candidates.) |
...
- **Spec G2 (provider-API VM provisioning).** Calls `provision_box(...)` to mint the seed, then submits it as userdata to the provider's create-VM API. Captures the resulting `public_ip` and calls `ru-box-mark-live`. Target providers must operate inside Russia or in CIS-adjacent jurisdictions that remain reachable from inside the GFW/TSPU — Selectel, Timeweb, FirstVDS, VK Cloud, Reg.ru, Yandex Cloud, etc. **Western hyperscalers (Hetzner, AWS, GCP, DigitalOcean) are explicitly out of scope: they do not operate in cordoned RU environments.** May add `--auto-provision` to `provision-seed` once G2 lands.
