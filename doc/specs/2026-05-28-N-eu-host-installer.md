...
| 5 | `standby-heartbeat-check` | replaces `preflight`: `standby-heartbeat-check --json` must show a recent active heartbeat; abort if the standby cannot see the active (mis-configured `[backup]`) |
...
| 7 | `standby-heartbeat-timer` | install + enable `mthydra-standby-heartbeat.timer` (skipped if promoted — a promoted node gets the active 8-phase maintenance timers instead) |
| 8 | `summary` | standby-specific TODOs: run `eu-node-add` on the active for this node (§1.11); if promoted, complete the printed Case-B manual rotations and re-run `obs-alert-test` to confirm sinks |

If `--promote`, phase 7 is replaced by the active `maintenance-timers` phase (#8 from §4.1).
