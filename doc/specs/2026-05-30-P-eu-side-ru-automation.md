...
> **Superseded in part by spec T2 (2026-06-05).** The *steady-state* runner is still key-only (the P-D5 password rationale stands for the 30-min loop). But the per-vantage `<vantage_id>.key` is replaced by a single shared key stored in the DB (`controller_probe_key`), and `vantage-setup` now also accepts a **one-time** interactive password to *open* the vantage for initial provisioning — safe because it is typed on the TTY (never argv/env/log) and made inert seconds later when the vantage is locked to `probe`-key-only. See `doc/specs/2026-06-05-T2-vantage-ssh-hardening.md` (T2-D4/T2-D5).
