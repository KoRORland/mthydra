...
| O-D4 | **`default_shard` is seeded at bootstrap.** The schema migration creates a shard with `shard_id='default_shard'` (empty members, default `target_size`) on fresh init and, idempotently, on upgrade if absent. | "Always exists from the start" — nothing has to lazily create it; provisioning and onboarding can assume it. |
| O-D5 | **Box→shard binds at provisioning.** `provision_box` assigns `default_shard` unless `--shard <S>` is passed. | Every box has a shard from creation → check 33 is unviolatable, and the box lands where users in that shard will look. |
